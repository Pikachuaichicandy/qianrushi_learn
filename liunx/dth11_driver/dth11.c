...
{
    /* 尝试多种方式查找设备树节点 */
    dth11.nd = of_find_node_by_path("/gpioled");
    if(dth11.nd == NULL) {
        printk("dth11: of_find_node_by_path failed \n");
        dth11.nd = of_find_node_by_name(NULL, "gpioled");
    }
    if(dth11.nd == NULL) {
        printk("dth11: of_find_node_by_name failed \n");
        dth11.nd = of_find_compatible_node(NULL, NULL, "alientek,gpioled");
    }
    if(dth11.nd == NULL) {
        printk("dth11: failed to find gpioled node in device tree\n");
        return -EINVAL;
    }

    dth11.gpio_dth11 = of_get_named_gpio(dth11.nd, "led-gpios", 0);
    ...
    gpio_request(dth11.gpio_dth11, "led-gpios");
