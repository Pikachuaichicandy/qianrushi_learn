...
struct hc_sr04_device {
    ...
    atomic_t echo_captured;             /* 判断等待条件标志 */
    ktime_t start_time, end_time;       /* 计时变量 */
    atomic_t lock_flag;                 /* 原子锁标志 */
};
...
static irqreturn_t hc_sr04_irq_handler(int irq, void *dev_id)
{
    ...
    if(echo_value){
        while (atomic_cmpxchg(&hc_sr04.lock_flag, 0, 1) != 0);
        hc_sr04.start_time = ktime_get();
        atomic_set(&hc_sr04.lock_flag, 0);
    }else{                                              /* 下降沿触发获取结束时间 */
        while (atomic_cmpxchg(&hc_sr04.lock_flag, 0, 1) != 0);
        hc_sr04.end_time = ktime_get();
        atomic_set(&hc_sr04.echo_captured, 1);           /* 设置标志位，表示已捕获到echo信号 */
        atomic_set(&hc_sr04.lock_flag, 0);
...
static long hc_sr04_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    atomic_set(&hc_sr04.echo_captured, 0);  /* 标志位置为0 */
    
    atomic_set(&hc_sr04.lock_flag, 0);
    
    /* 等待下降沿中断唤醒等待队列并且设置超时时间100ms */
    ret = wait_event_interruptible_timeout(hc_sr04.wait_queue, atomic_read(&hc_sr04.echo_captured), msecs_to_jiffies(100));
    if(ret == 0) {
...
static int hc_sr04_ioinit(void)
{
    ...
    /* 获取设备树节点 */
    hc_sr04.node = of_find_node_by_path("/hc-sr04");
    if (!hc_sr04.node) {
...
static int hc_sr04_probe(struct platform_device *dev)
{
    ...
    /* 初始化原子锁标志 */
    atomic_set(&hc_sr04.lock_flag, 0);
    atomic_set(&hc_sr04.echo_captured, 0);
