...
void MainScene::paintEvent(QPaintEvent *event)
{
    ...
    {
        paintFood(&painter); //后面绘制可以保证蛇在图像上面
        paintSnakeBody(&painter);
    }
    else
        painterGameOver(&painter);
    painBorder(&painter);
}
...
{
    QPixmap temp=QPixmap(":/images/food/drug.png").scaled(BWidth,BWidth); //scaled缩放图片
    for(int i=drugCount;i<appleCount+drugCount;i++)
    {
        totalFood[i]=createFood();
...
void MainScene::updateScore(int score) {
    // 更新分数的逻辑
    m_score = score;
    update(); // 重新绘制，显示新的分数
}
