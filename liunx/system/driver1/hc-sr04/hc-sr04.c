...
static int hc_sr04_ioinit(void)
{
    ...
    // /* 获取设备树节点 */
    // hc_sr04.node = of_find_node_by_path("/gpioled");
    // if (!hc_sr04.node) {
    //     printk("hc_sr04: device tree node not found!\n");
    //     return -ENODEV;
    // }
    /* 尝试多种方式查找设备树节点 */
   hc_sr04.node = of_find_node_by_path("/sr04");
    if(hc_sr04.node == NULL) {
        printk("hc_sr04: of_find_node_by_path failed \n");
       hc_sr04.node = of_find_node_by_name(NULL, "sr04");
    }
    if(hc_sr04.node == NULL) {
        printk("hc_sr04: of_find_node_by_name failed \n");
       hc_sr04.node = of_find_compatible_node(NULL, NULL, "jgb,sr04");
    }
    if(hc_sr04.node == NULL) {
        printk("hc_sr04: failed to find gpioled node in device tree\n");
        return -EINVAL;
    }
