...
void SnakeWidget::recvData(bool isSelf,int posi,Point point,Point food,QString name,int head,int body)
{
    ...
    //设置用户数据
    if(m_userWidget[posi]==nullptr)
        m_userWidget[posi]=new UserWidget(120,95,this);
    m_userWidget[posi]->setVariable(username[posi]+"   ",userScore[posi],headSkin[posi][2],bodySkin[posi]);
    m_userWidget[posi]->move(posi*121,MHeight+5);
    m_userWidget[posi]->show();

    totalUser++;
    if(totalUser==userNum)
    {
        m_userWidget[m_position]->setFontColor();
        m_timer->start();
    }
}
...
void SnakeWidget::recvFoodPoint(Point p)
{
    if(m_food.x<0 || m_food.y<0)
    {
        m_food.x=p.x,m_food.y=p.y;
        qDebug()<<"重置食物坐标";
    }
}
...
void SnakeWidget::isEatFood(Food& food)
{
    for(int i=0;i<userNum;i++)
    {
        ...
        if(snake[i][0].x==food.x&&snake[i][0].y==food.y)
        {
            ...
            userScore[i]++;   //分数加1
            addBody(i);
            m_userWidget[i]->setScore(userScore[i]);
            break;
        }
    }
}
