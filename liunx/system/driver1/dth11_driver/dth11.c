...
{
    /* 优先使用兼容性字符串查找设备树节点 */
    dth11.nd = of_find_compatible_node(NULL, NULL, "jgb,dht11");
    if(dth11.nd == NULL) {
        printk("dth11: of_find_compatible_node failed \n");
        /* 备用查找方法 */
        dth11.nd = of_find_node_by_name(NULL, "dht11");
    }
    if(dth11.nd == NULL) {
        printk("dth11: of_find_node_by_name failed \n");
        dth11.nd = of_find_node_by_path("/dht11");
    }
    if(dth11.nd == NULL) {
        printk("dth11: failed to find dht11 node in device tree\n");
        return -EINVAL;
    }

    dth11.gpio_dth11 = of_get_named_gpio(dth11.nd, "dht11-gpios", 0);
    ...
    gpio_request(dth11.gpio_dth11, "dht11-gpios");
