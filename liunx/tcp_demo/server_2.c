...
// 将 cli_data_handle 函数定义移到 main 函数之前
void cli_data_handle(void *arg)
{
    int newfd = *(int *)arg; // 获取传入的newfd
    printf("handler_pthread_id: %ld, fd: %d\n", pthread_self(), newfd);
...
int main(void)
{
    ...
    while(1)
    {
        ...
        char ipv4_addr[16];
        // 修正比较逻辑
        if(inet_ntop(AF_INET, (void *)&client_addr.sin_addr, ipv4_addr, sizeof(ipv4_addr)) == NULL)
        {
            perror("inet_ntop");
            exit(-1);
        }
        ...
        if(pthread_create(&tid, NULL, (void *)cli_data_handle, (void *)&newfd) != 0)
        {
            perror("pthread_create");
            exit(-1);
        }
        pthread_detach(tid); // 分离线程，避免资源泄露
    }
}
