...
static int hc_sr505_ioinit(void)
{
    ...
    /* 获取设备树节点 */
    hc_sr505.node = of_find_node_by_path("/sr505");
    if (!hc_sr505.node) {
    ...
    /* 获取Trig引脚GPIO编号 */
    hc_sr505.out_gpio = of_get_named_gpio(hc_sr505.node, "sr505-gpios", 0);
    ...
    /* 请求GPIO */    
    ret = gpio_request(hc_sr505.out_gpio, "sr505-gpios");
    if(ret < 0) {
